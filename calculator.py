...
def get_best_combo(hand: Deck, deck: Deck, num_hands: int) -> Deck:
    ...
    for combo_to_play in combos_to_play:
        ...
        combos_from_deck = combinations(deck.cards, 5)
        for combo_from_deck in combos_from_deck:
            score = get_score(Deck(list(combo_to_play)))
            hand.discard(combo_to_play)
            hand.add(combo_from_deck)
            deck.discard(combo_from_deck)
            score += get_score(get_best_combo(hand, deck, num_hands - 1)) / len(list(combos_from_deck))
            deck.add(combo_from_deck)
            if (score > max_score):
                max_score = score
                best_combo = Deck(list(combo_to_play))
            hand.discard(combo_from_deck)
            hand.add(combo_to_play)
    print(max_score)
    return best_combo
...
def main():
    ...
    print(get_best_combo(Deck(hand), Deck(deck), num_hands))
